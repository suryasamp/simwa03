SIMWA (Sistem Informasi Manajemen Warga) adalah aplikasi berbasis Spring MVC + jQuery yang dirancang untuk membantu pengelolaan data warga, pembayaran iuran, serta pencatatan transaksi secara terintegrasi.
Aplikasi ini mendukung integrasi dengan Midtrans Payment Gateway untuk proses pembayaran online, serta menggunakan PostgreSQL sebagai basis data utama.

🔧 Fitur Utama
    
    ◉ Manajemen data warga (CRUD)
    ◉ Pembayaran iuran melalui Midtrans (Snap API)
    ◉ Validasi transaksi menggunakan signature_key
    ◉ Dashboard administrasi
    ◉ Tampilan interaktif menggunakan jQuery dan AJAX
    ◉ Struktur modular berbasis Spring MVC

🧱 Teknologi yang Digunakan
    
    ◉ Backend: Java, Spring MVC, Spring Boot (opsional)
    ◉ Frontend: HTML, CSS, Bootstrap, jQuery
    ◉ Database: PostgreSQL
    ◉ Integrasi: Midtrans Payment Gateway
    ◉ Version Control: Git & GitHub

🚀 Tujuan

Menyediakan sistem informasi yang efisien, transparan, dan mudah digunakan untuk pengelolaan iuran dan data warga di lingkungan masyarakat.
